...
describe("SharedRuntimeV2PaneBridge", () => {
  ...
  test("records terminal bandwidth and queue pressure stats for patch viewers", async () => {
    ...
    await expect.poll(() => bandwidthTracker.getStats().snapshotBytesSent).toBeGreaterThan(0);
    await expect.poll(() => bandwidthTracker.getStats().streamBytesSent).toBeGreaterThan(0);
    expect(bandwidthTracker.getStats().staleRevisionDrops).toBeGreaterThanOrEqual(0);
    await expect.poll(() => bandwidthTracker.getStats().replayToLiveTransitions).toBeGreaterThan(0);
